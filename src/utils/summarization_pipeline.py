...
class SummarizationPipeline:
    def __init__(
        ...
        in_8bit: bool = False,
        # default prefix for models like BART/T5, for pegasus it is not required
    ...
    def _load_model_and_tokenizer(self):
        ...
        self.logger.info(f"Loading model for {self.model_name_or_path}" f" (8bit={self.in_8bit}, device={self.device})")
    ...
    def tokenize_dataset(
        ...
        return dataset.map(
            lambda examples: self.preprocess(examples[text_column], examples.get(summary_column, None)), batched=batched
        )

    def init_trainer(self, training_args: Seq2SeqTrainingArguments, train_dataset: Dataset, eval_dataset: Dataset):
    ...
    def summarize(self, texts: List[str], max_new_tokens: int = 100, **generate_kwargs) -> List[str]:
